...
(function(exports) {
	...
	DroneServer.prototype.saveClient = function(socket) {
		...
		if(this.clients[socket.id] == socket)
			return;
		...
		// internal transmit event
		var transmit = socket.id + '_transmit';
		...
		socket.$emit = function() {
			...
			// remove first argument if newListener
			if(args[0] === 'newListener')
				newListener = args.shift();

			//retransmit as internal transmit
			args.unshift(transmit);
			if(newListener)
				args.unshift(newListener);
