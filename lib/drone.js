...
(function(exports) {
	...
	DroneConnection.prototype.authenticate = function(socket, data, callback) {
 	 ...
 	 * @api private
 	 */
 	DroneConnection.prototype.connectStreams = function(sender, reciever, events) {
 		
 		for(var i=0; i<events.length; i++) {
 			var evt = events[i];
 			var args = Array.prototype.unshift.call(arguments, evt);

 			sender.on(evt, function() {
 				reciever.emit.apply(_this, args);
 			})
 		}

 		var _this = this;

 		this.droneSocket.on('data', function() {
 			_this.clientSocket.emit.apply(null, arguments);
 		});

 		this.clientSocket.on('data', function() {
 			_this.droneSocket.emit.apply(null, arguments);
 		});
 	};
